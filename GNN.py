...
def split(data_list, train_p, val_p, test_p):
    ...
    class_splits = [
        int(train_p * 160), int(train_p * 160), int(train_p * 160),
        int(val_p * 160), int(val_p * 160), int(val_p * 160),
        int(test_p * 160), int(test_p * 160), int(test_p * 160)
    ]
...
class GCN(torch.nn.Module):
    def __init__(self, hidden_channels):
        ...
        torch.manual_seed(12345)
        self.conv1 = GCNConv(3, hidden_channels) #num node features
