...
3D CNN based on EfficientNetV2 using spectrogram images.
"""
import os
...
def makemodel(S1,batch_size,epoch,train_p, val_p, test_p): 
    ...
    def makespect(S1):
        ...
        yl = np.zeros([1,np.size(subject_data['S1'],1)])
        yr = np.ones([1,np.size(subject_data['S1'],1)])
        yre = np.ones([1,np.size(subject_data['S1'],1)])*2
...
accuracies, model = makemodel(subject_data['S1'][:,:],1,20,0.3,0.2,0.5)